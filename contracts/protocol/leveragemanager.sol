...
contract LeverageManager is ERC721Enumerable {
    ...
    struct LeveredBond {
        uint256 debt; //how much underlying owed from vault
    ...
    function getPosition(uint256 marketId, address trader)
        public
        view
        returns (LeveredBond memory)
    {
    ...
    function issuePerpBondLevered(
        ...
        require(
            _leverage <= getMaxLeverage(msg.sender) && _leverage >= precision,
            "!leverage"
        );
        ...
        marketManager.transferTraderCap(
            address(underlying),
            msg.sender,
            address(this),
            amountPulled
        ...
        issueQTY = marketManager.issueBond(
            _marketId,
            _amountIn,
            address(this),
            msg.sender
        ...
        leveragePosition[_marketId][msg.sender].debt += (_amountIn -
            amountPulled);
        leveragePosition[_marketId][msg.sender].amount += (issueQTY);

        emit LeveredBondIssued(
            _marketId,
            msg.sender,
            _amountIn,
            issueQTY,
    ...
    function redeemLeveredPerpLongZCB(uint256 marketId, uint256 redeemAmount)
        external
        returns (
        ...
        )
    {
        ...
        (collateral_redeem_amount, ) = marketManager.redeemPerpLongZCB(
            marketId,
            redeemAmount,
            address(this),
            msg.sender
        );
        vars.vault.UNDERLYING().transfer(
            address(vars.vault),
        ...
        if (position.debt == 0) {
            //100-70 = 30 send me back 30!!
            postRepayLeftOver = collateral_redeem_amount - paidDebt;
            vars.vault.withdrawFromPoolInstrument(
                marketId,
                postRepayLeftOver,
                msg.sender,
                0,
    ...
    function buyBondLevered(
        ...
        uint256 amountPulled = _amountIn.divWadDown(_leverage);
        // transfer amountPulled to LM, but give allowance to MM.
        marketManager.transferTraderCap(
            address(underlying),
            msg.sender,
            address(this),
        ...
        // pull underlying funds from vault to this address.
        controller.pullLeverage(_marketId, _amountIn - amountPulled);
        ...
        // LM receives longZCB.
        (amountIn, amountOut) = marketManager.buylongZCB(
            _marketId,
            int256(_amountIn),
            _priceLimit,
            emptyByte,
    ...
    function redeemLeveredBond(uint256 marketId) public {
        ...
        uint256 collateral_redeem_amount = collateral_back >=
            uint256(position.debt)
            ? collateral_back - uint256(position.debt)
            : 0;
    ...
    event LeveredBondRedeemDenied(
        uint256 indexed marketId,
        address indexed trader
    );

    function redeemDeniedLeveredBond(uint256 marketId)
        public
    ...
    function addLeveragePool(uint256 vaultId, address pool) public //onlyowner
    {
    ...
    function _mintWithLeverage(
        ...
        uint256 maxBorrowableAmount = min(
            borrowAmount,
            vars.collateralPower.mulWadDown(collateralAmount)
        );
        ...
        vars.shares = vars.vault.deposit(
            vars.maxBorrowableAmount,
            address(this)
        );
    }
    ...
    function mintWithLeverage(
    ...
    ) public returns (uint256 tokenId, Position memory newPosition) {
        MintLocalVars memory vars;
        ...
        underlying.approve(
            address(vars.vault),
            suppliedCapital.mulWadDown(precision + leverageFactor)
        );
    ...
    function rewindPartialLeverage(
        ...
        underlying.approve(
            ...
            vault.previewMint(withdrawAmount)
        ); //TODO
        ...
        while (vars.withdrawAmount != 0) {
            vars.sharesRedeemed = min(
                position.endStateBalance,
                vars.withdrawAmount
            );
            ...
            leveragePool.repay(
                leveragePool.toBorrowShares(vars.assetReturned, true),
                address(this)
            );
            ...
            vars.removed =
                ((_maxBorrowableAmount -
                    leveragePool.toBorrowAmount(
                        leveragePool.userBorrowShares(address(this)),
                        true
                    )) * 1e18) /
                perUnitMaxBorrowAmount;
            ...
            console.log(
                "redeemed shares",
                min(position.endStateBalance, vars.withdrawAmount)
            );
            ...
            position.endStateBalance = position.endStateBalance >=
                vars.withdrawAmount
        ...
        if (position.borrowedCapital >= vars.totalAssetReturned)
            ...
            position.suppliedCapital -=
                vars.totalAssetReturned -
                position.borrowedCapital;
        }
    ...
    function getPositions(address _owner)
        public
        view
        returns (Position[] memory)
    {
        uint256[] memory ids = getTokenIds(_owner);
