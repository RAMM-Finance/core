...
//         vm.prank(jonna); 
//         LeverageModule.Position memory position = leverageModule.mintWithLeverage(1, suppliedCapital, leverageFactor); 
...
//         assertApproxEqAbs(position.borrowedCapital, cBal(address(pool))- vars.cbalnow, 10);  
//         assertEq(position.borrowedCapital, leverageFactor.mulWadDown(suppliedCapital));
//         assertApproxEqAbs(Vault(vars.vault_ad).totalSupply() - vars.amountIn, vars.)
//     }
